...
> you may want to use
> [FuncGodot](https://github.com/func-godot/func_godot_plugin) or
> [Cyclops Level
> Builder](https://github.com/blackears/cyclopsLevelBuilder) instead.

::: {.video alt="CSG being used to subtract a torus shape from a box" autoplay="" loop="" muted=""}
...
Any mesh can be used for `CSGMesh3D <class_CSGMesh3D>`{.interpreted-text
role="ref"}; the mesh can be modeled in other software and imported into
Godot. Multiple materials are supported. There are some restrictions for
geometry:
...
> adding objects together (such as table and room objects), create them
> as separate CSG trees. Forcing too many objects in a single tree will
> eventually start affecting performance. Only use binary operations
...
> [!TIP]
> Working in **Orthogonal** projection gives a better view when
> combining the CSG shapes.
...
> [!NOTE]
> This is to illustrate the effect of CSG processing order. Since the
> root node is not a CSG node, the CSGCombiner3D nodes are the end of
> the operations; this shows the use of CSGCombiner3D to organize the
> CSG scene.
...
> Inspector and choose **Copy**. To paste it, select the node you\'d
> like to apply the material onto, click the dropdown arrow next to its
